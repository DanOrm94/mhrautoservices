  ...
  function setupRecentWorkSlideshow() {
    ...
    gallery.innerHTML = `
      <div class="mhr-slide-stage">
        ${images.map((src, i) => `<img class="mhr-slide ${i === 0 ? 'active' : ''}" src="${src}" alt="MHR Auto Services recent work ${i + 1}" loading="${i === 0 ? 'eager' : 'lazy'}">`).join('')}
      ...
      <div class="mhr-slide-footer">
        ...
        <div class="mhr-dots" aria-label="Choose image">
          ${images.map((_, i) => `<button type="button" class="mhr-dot ${i === 0 ? 'active' : ''}" aria-label="Show image ${i + 1}"></button>`).join('')}
        </div>
